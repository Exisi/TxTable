...
const model = {
	HTML: () => {
		const list = textDeepIteration(attr.input, attr.rule.split);
		const output = textFormatWithHTML(list, attr.rule);
		return output;
	},
	Markdown: () => {
		const list = textDeepIteration(attr.input, attr.rule.split);
		const output = textFormatWithMarkdown(list, attr.rule);
		return output;
	},
};
...
function getModel(checkbox) {
	...
	for (const check of checkbox) {
		if (!check.nodeType > 0 || !check.checked) {
			continue;
		}
...
function textDeepIteration(input, split) {
	...
	for (const row of str.list.by_row) {
		const in_row_word_list = row.split(str.split);
...
function textFormatWithMarkdown(list, rule) {
	...
	const max_column = list[0].length;
	...
	const separator = new Array(2 * max_column + 1).fill("|");
...
function restoreListToText(list) {
	/* list flatten */
	const list_flatten = list.flat(Infinity);
